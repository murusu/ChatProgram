...
class UserObject
{
	...
	public:
        ...
        struct stServer stServer;
        struct stHost stHost;
		struct stTarget stTarget;


        void   vGetHostID (HWND hName, HWND hPassWord);
        void   vGetServerInfo (HWND hIP, HWND hPort);
        struct sockaddr_in stGetTargetAddr(void);
        bool   bSetGroupTarget(HWND hGroupName);

		void vInitSocketInfo(void);
		void vInitUserList(void);
		void vInitGroupList(void);
		void vInitServer(void);
		void vInitHost(void);
		void vInitTarget(void);
		void vCleanSocket(int iSocketNo);
        int  iGetSocket(void);		
		int  iFindUserFromSocketNo(int iSocketNo);
		int  iFindUserFromAddr(struct sockaddr_in stSockaddr);
		int  iFindUserFromUserName(char * szUserName);
